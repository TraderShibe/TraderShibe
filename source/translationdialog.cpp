	...
	: QDialog(parent)
{
	...
	ui.deleteTranslationButton->setEnabled(!ShibeTranslator.lastFile().startsWith(":/Resource"));

	ui.languageName->setText(ShibeTr("LANGUAGE_NAME","Invalid"));
	authorAbout=new TranslationLine;
	ui.authorLayout->addWidget(authorAbout);
	authorAbout->setItemText(ShibeTr("LANGUAGE_AUTHOR","Invalid"));

	gridLayout=new QGridLayout;
	fonWidget.setLayout(gridLayout);
	ui.scrollArea->setWidget(&fonWidget);

	ShibeTranslator defaultTranslation;
	defaultTranslation.loadFromFile(":/Resources/Language/English.lng");

	fillLayoutByMap(&(ShibeTranslator.labelMap),"Label_",&(defaultTranslation.labelMap));
	fillLayoutByMap(&(ShibeTranslator.groupBoxMap),"GroupBox_",&(defaultTranslation.groupBoxMap));
	fillLayoutByMap(&(ShibeTranslator.checkBoxMap),"CheckBox_",&(defaultTranslation.checkBoxMap));
	fillLayoutByMap(&(ShibeTranslator.buttonMap),"Button_",&(defaultTranslation.buttonMap));
	fillLayoutByMap(&(ShibeTranslator.spinBoxMap),"SpinBox_",&(defaultTranslation.spinBoxMap));
	fillLayoutByMap(&(ShibeTranslator.stringMap),"String_",&(defaultTranslation.stringMap));
...
void TranslationDialog::deleteTranslationButton()
{
	...
	msgBox.setWindowTitle(ShibeTr("MESSAGE_CONFIRM_DELETE_TRANSLATION","Please confirm removing file"));
	msgBox.setText(ShibeTr("MESSAGE_CONFIRM_DELETE_TRANSLATION_TEXT","Are you sure to delete translation file?"));
	...
	if(QFile::exists(ShibeTranslator.lastFile()))QFile::remove(ShibeTranslator.lastFile());
	ui.deleteTranslationButton->setEnabled(QFile::exists(ShibeTranslator.lastFile()));
...
void TranslationDialog::applyButton()
{
	...
	for(int n=0;n<lineEdits.count();n++)
	{
		...
		{
			ui.buttonSaveAs->setEnabled(false);
			QMessageBox::warning(this,windowTitle(),ShibeTr("LANGUAGE_NOT_APPROVED","Please fill empty fields"));
			return;
		}
...
void TranslationDialog::saveAsButton()
{
	...
	QString fileName=QFileDialog::getSaveFileName(this, ShibeTr("SAVE_TRANSLATION","Save Translation"),QDesktopServices::storageLocation(QDesktopServices::DesktopLocation)+"/"+ui.languageName->text().replace("/","_").replace("\\","").replace(":","").replace("?","")+".lng","(*.lng)");
	if(fileName.isEmpty())return;
	if(QFile::exists(fileName))QFile::remove(fileName);
	QFile::copy(ShibeTranslator.lastFile(),fileName);
}
