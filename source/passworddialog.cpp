	...
	: QDialog(parent)
{
	...
	for(int n=0;n<settingsList.count();n++)
	{
		...
		if(baseValues.appVerLastReal<1.0775)
		{
			...
			{
				QFile::remove(appDataDir+settingsList.at(n));
				static bool havecryptsyProfile=false;
				if(!havecryptsyProfile)
				{
					havecryptsyProfile=true;
					QMessageBox::warning(0,windowTitle(),"From now cryptsy support API keys with permissions. To ensure the security of your cryptsy account you must create new API keys and add a new profile of cryptsy to the Qt Bitcoin Trader.");
				}
				continue;
			}
		}
		...
		if(!isProfLocked&&lastProfileIndex==-1&&lastProfile==settingsList.at(n))lastProfileIndex=n-1;
		if(firstUnlockedProfileIndex==-1&&!isProfLocked)firstUnlockedProfileIndex=n-1;
	}
	if(ui.profileComboBox->count()==0)ui.profileComboBox->addItem(ShibeTr("DEFAULT_PROFILE_NAME","Default Profile"));
...
{
	return QDesktopServices::storageLocation(QDesktopServices::TempLocation)+"/Trader Shibe_lock_"+QString(QCryptographicHash::hash(appDataDir+"/"+QFileInfo(name).fileName().toAscii(),QCryptographicHash::Sha1).toHex());
}
...
{
	int currIndex=ui.profileComboBox->currentIndex();
	if(currIndex==-1)return appDataDir+"Trader Shibe.ini";
...
void PasswordDialog::resetDataSlot()
{
	...
	msgBox.setText(ShibeTr("CONFIRM_DELETE_PROFILE","Are you sure to delete \"%1\" profile?").arg(ui.profileComboBox->currentText()));
