...
{
	ui.groupBoxApiKeyAndSecret->setTitle(ShibeTr("API_KEY_AND_SECRET","%1 API key and Secret").arg(name));
...
void NewPasswordDialog::updateIniFileName()
{
	...
	baseValues.iniFileName=appDataDir+"Trader Shibe.ini";
	else
	{
...
void NewPasswordDialog::okPressed()
{
	if(isValidPassword())accept();
		...
		QMessageBox::warning(this,"Qt Bitcoin Trader",ShibeTranslator.translateLabel("TR00100","Your password must be at least 8 characters and contain letters, digits, and special characters."));
}
