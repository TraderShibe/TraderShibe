	...
	: QDialog()
{
	...
	new ShibeSpinBoxFix(ui.thanValue);
	new ShibeSpinBoxFix(ui.thanValuePercentage);
	new ShibeSpinBoxFix(ui.btcValue);
	new ShibeSpinBoxFix(ui.exactPriceValue);

	ui.thanValueBack->layout()->addWidget(new ShibeSpinBoxPicker(ui.thanValue));
	ui.valuePercentageBackExecPrice->layout()->addWidget(new ShibeSpinBoxPicker(ui.valueCheckValuePercentage));
	ui.percentageLayout->addWidget(new ShibeSpinBoxPicker(ui.thanValuePercentage,0,0.01));
	ui.valuePercentageBack100->layout()->addWidget(new ShibeSpinBoxPicker(ui.valueValuePercentage,0,0.01));
	ui.exactPricePickerLayout->addWidget(new ShibeSpinBoxPicker(ui.exactPriceValue));
	ui.amountLayoutPicker->addWidget(new ShibeSpinBoxPicker(ui.btcValue));
	...
	connect(&(baseValues_->ShibeTranslator_),SIGNAL(languageChanged()),this,SLOT(languageChanged()));
...
void AddRuleWindow::languageChanged()
{
	...
	ui.checkBtcBalance->setText(ShibeTr("IF_BALANCE","%1 Balance").arg(curA));
	ui.checkUsdBalance->setText(ShibeTr("IF_BALANCE","%1 Balance").arg(curB));

	ui.checkSellAllIn->setText(ShibeTr("SELL_ALL_MY","Sell All my %1").arg(curA));
	ui.checkSellHalfIn->setText(ShibeTr("SELL_HALF_MY","Sell Half my %1").arg(curA));

	ui.checkBuyAllIn->setText(ShibeTr("SPEND_ALL_MY","Spend All my %1").arg(curB));
	ui.checkBuyHalfIn->setText(ShibeTr("SPEND_HALF_MY","Spend Half my %1").arg(curB));
...
void AddRuleWindow::buttonAddRule()
{
	...
	else QMessageBox::warning(this,windowTitle(),ShibeTr("INVALID_RULE_CHECK","This rule will be executed instantly.<br>This means that you make a mistake.<br>Please check values you entered."));
}
...
void AddRuleWindow::amountChanged()
{
	if(ui.checkSendEmail->isChecked()||ui.checkExecuteCommand->isChecked())
	{
		...
		ShibeTranslator.translateUi(&featureNotAvailable);
	...
	if(changingSound==false&&ui.checkPlaySoundWav->isChecked()&&(sound.isEmpty()||sound=="Beep"))
	{
		...
		sound=QFileDialog::getOpenFileName(this, ShibeTr("OPEN_SOUND_FILE","Open WAV file"), lastOpenFile, "Sound File (*.wav)");
...
{
	ui.addNewRuleGroupbox->setTitle(ShibeTr("EDIT_RULE","Edit rule"));
