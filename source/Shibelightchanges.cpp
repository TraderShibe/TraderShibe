...
ShibeLightChanges::ShibeLightChanges(QDoubleSpinBox *parent)
	: QObject()
{
	lastValue=0.0;
...
ShibeLightChanges::~ShibeLightChanges()
{
	if(changeTimer)delete changeTimer;
}

void ShibeLightChanges::changeTimerSlot()
{
	parentSpinBox->setStyleSheet("");
}

void ShibeLightChanges::valueChanged(double val)
{
	changeTimer->stop();
	if(lastValue<=val)
