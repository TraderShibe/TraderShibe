...
int main(int argc, char *argv[])
{
	...
	a.setApplicationName("Trader Shibe");
...
#ifdef Q_OS_WIN
	{
	QString appLocalStorageDir=a.applicationDirPath()+QLatin1String("/Trader Shibe/");
	...
	{
		ShibeTranslator.loadFromFile(baseValues.defaultLangFile);
...
#else
    appDataDir=QDesktopServices::storageLocation(QDesktopServices::DataLocation).replace("\\","/").toAscii()+"/";
	QString oldAppDataDir=QDesktopServices::storageLocation(QDesktopServices::HomeLocation).toAscii()+"/.config/Trader Shibe/";
	...
	{
		QFile::rename(appDataDir+"/Settings.set",appDataDir+"/Trader Shibe.cfg");
	}
    ...
    if(argc>1)
	{
        if(a.arguments().last().startsWith("/checkupdate"))
		{
			...
			ShibeTranslator.loadFromFile(langFile);
	...
	a.setWindowIcon(QIcon(":/Resources/Trader Shibe.png"));
	QFile *lockFile=0;
	...
	baseValues.logFileName=QLatin1String("Trader Shibe.log");
	baseValues.iniFileName=QLatin1String("Trader Shibe.ini");
	...
	while(tryDecrypt)
	{
		...
		if(!tryPassword.isEmpty())
		{
			...
			if(profileLocked)
			{
				...
				msgBox.setText(ShibeTr("THIS_PROFILE_ALREADY_USED","This profile is already used by another instance.<br>API does not allow to run two instances with same key sign pair.<br>Please create new profile if you want to use two instances."));
#ifdef Q_OS_WIN
			...
			if(!profileLocked)
			{
				...
				QStringList decryptedList=QString(ShibeAES256::decrypt(QByteArray::fromBase64(settings.value("EncryptedData/ApiKeySign","").toString().toAscii()),tryPassword.toAscii())).split("\r\n");
	...
	baseValues.mainWindow_=new Trader Shibe;
