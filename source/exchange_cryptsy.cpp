	...
	: Exchange()
{
	...
	lastTradesDate=0;
	tickerLastDate=0;
	baseValues.exchangeName="Cryptsy";//Sorry, Cryptsy code is not published yet. I'll make update soon
...
void Exchange_Cryptsy::sendToApi(int reqType, QByteArray method, bool auth, bool sendNow, QByteArray commands)
{
	...
	if(auth)
	{
		...
		if(sendNow)
		ShibeHttp->sendData(reqType, "POST /api/2/"+method,postData, "Rest-Sign: "+hmacSha512(privateRestSign,forHash).toBase64()+"\r\n");
		else
		ShibeHttp->prepareData(reqType, "POST /api/2/"+method, postData, "Rest-Sign: "+hmacSha512(privateRestSign,forHash).toBase64()+"\r\n");

	}
	else
	{
		if(sendNow)
			ShibeHttp->sendData(reqType, "GET /api/2/"+method);
		else 
			ShibeHttp->prepareData(reqType, "GET /api/2/"+method);
	}
}
