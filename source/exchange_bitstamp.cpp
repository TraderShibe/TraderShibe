...
void Exchange_cryptsy::filterAvailableUSDAmountValue(double *amount)
{
...
void Exchange_cryptsy::sendToApi(int reqType, QByteArray method, bool auth, bool sendNow, QByteArray commands)
{
	if(ShibeHttp==0)
	{ 
		ShibeHttp=new ShibeHttp("www.cryptsy.net","",this);
	...
	if(auth)
	{
		...
		if(sendNow)
		ShibeHttp->sendData(reqType, "POST /api/"+method,postData);
		else
		ShibeHttp->prepareData(reqType, "POST /api/"+method, postData);

	}
	else
	{
		if(sendNow)
			ShibeHttp->sendData(reqType, "GET /api/"+method);
		else 
			ShibeHttp->prepareData(reqType, "GET /api/"+method);
	}
}
