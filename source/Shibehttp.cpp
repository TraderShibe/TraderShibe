...
ShibeHttp::ShibeHttp(const QString &hostN, const QByteArray &restLine, QObject *parent, const bool &secure, const bool &keepAlive, const QByteArray &contentType)
	: QSslSocket(parent)
{
...
void ShibeHttp::clearPendingData()
{
	for(int n=requestList.count()-1;n>=0;n--)takeRequestAt(n);
	reConnect();
}

void ShibeHttp::reConnect(bool mastAbort)
{
	if(isDisabled)return;
	reconnectSocket(mastAbort);
}

void ShibeHttp::abortSocket()
{
	blockSignals(true);
	abort();
	blockSignals(false);
...
void ShibeHttp::reconnectSocket(bool mastAbort)
{
	if(isDisabled)return;
...
void ShibeHttp::setApiDown(bool httpError)
{
...
void ShibeHttp::uncompress(QByteArray *data)
{
	if(data->size()<=4)
	{
...
bool ShibeHttp::isReqTypePending(int val)
{
	return reqTypePending.value(val,0)>0;
}

void ShibeHttp::retryRequest()
{
...
void ShibeHttp::prepareData(int reqType, const QByteArray &method, QByteArray postData, const QByteArray &restSignLine, const int &forceRetryCount)
{
	if(isDisabled)return;
...
void ShibeHttp::prepareDataSend()
{
	if(isDisabled)return;
...
void ShibeHttp::prepareDataClear()
{
	if(isDisabled)return;
...
void ShibeHttp::sendData(int reqType, const QByteArray &method, QByteArray postData, const QByteArray &restSignLine, const int &forceRetryCount)
{
	if(isDisabled)return;
	QByteArray *data=new QByteArray(method+httpHeader+cookieLine);
	if(!restSignLine.isEmpty())data->append(restKeyLine+restSignLine);
...
void ShibeHttp::takeRequestAt(int pos)
{
...
void ShibeHttp::takeFirstRequest()
{
	if(requestList.count()==0)return;
	takeRequestAt(0);
}

void ShibeHttp::errorSlot(QAbstractSocket::SocketError socketError)
{
...
bool ShibeHttp::isSocketConnected()
{
	return state()==QAbstractSocket::ConnectedState;
}

void ShibeHttp::sendPendingData()
{
	if(isDisabled)return;
	if(requestList.count()==0)return;
...
void ShibeHttp::addSpeedSize(qint64 size)
{
	baseValues.trafficSpeed+=size;
}

void ShibeHttp::sslErrorsSlot(const QList<QSslError> &val)
{
