	...
	: QDialog()
{
	...
	if(useOldUpdateEngine)httpGet=new ShibeHttp("raw.github.com",0,this,true,false);
	else httpGet=new ShibeHttp("qbtapi.centrabit.com",0,this,false,false);
	...
	{
	if(baseValues.appVerIsBeta)httpGet->sendData(320,"GET /ShibeShibe/Trader Shibe/master/versionsbeta.txt");
	else	httpGet->sendData(320,"GET /ShibeShibe/Trader Shibe/master/versions.txt");
	}
	...
	if(stateUpdate==0)
	{
		...
		if(updateVersion.toDouble()<=baseValues.appVerReal)
		{
			if(feedbackMessage)
			{
				...
				msgb.setText(ShibeTr("UP_TO_DATE","Your version of Qt Bitcoin Trader is up to date."));
				msgb.exec();
			}
			exitSlot();
			return;
		...
		ui.iconLabel->setPixmap(QPixmap(":/Resources/Trader Shibe.png"));
		...
		if(stateUpdate==1)
		{
			...
			QByteArray decrypted=ShibeRSA::getSignature(updateSignature,publicKey);
			if(decrypted==fileSha1)
			{
				...
				if(!QFile::exists(curBin)){QMessageBox::critical(this,windowTitle(),"Critical error. Please reinstall application. Download it from http://sourceforge.net/projects/Trader Shibe/<br>File not exists: "+curBin+"<br>"+updBin);downloadError(5);return;}
#ifdef Q_OS_MAC
				QFile(curBin).setPermissions(QFile(bkpBin).permissions());
#endif
				QMessageBox::information(this,windowTitle(),ShibeTr("UPDATED_SUCCESSFULLY","Application updated successfully. Please restart application to apply changes."));
				QSettings settings(appDataDir+"/Trader Shibe.cfg",QSettings::IniFormat);
...
void UpdaterDialog::buttonUpdate()
{
	...
	httpGet=new ShibeHttp(domain,0,this,protocol.startsWith("https"),false);
...
void UpdaterDialog::downloadError(int val)
{
	...
	QMessageBox::warning(this,windowTitle(),ShibeTr("DOWNLOAD_ERROR","Download error. Please try again.")+"<br>"+httpGet->errorString()+"<br>CODE: "+QString::number(val));
